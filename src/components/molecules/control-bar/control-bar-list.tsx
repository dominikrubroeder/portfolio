...
export default function ControlBarList({
  ...
  return (
    <div
      className={cn(
        'no-scrollbar absolute bottom-20 z-50 w-[95vw] max-w-(--breakpoint-sm) items-center space-y-4 rounded-2xl border bg-background p-3 px-4 transition md:visible md:relative md:top-4 md:flex md:w-max md:max-w-[85svw] md:gap-4 md:space-y-0 md:overflow-x-auto md:rounded-full lg:max-w-none',
    ...
    >
      {sections.map((section) => (
        <button
          key={section.id}
          className={cn(
            'group relative block interactive rounded-full px-3 py-1.5 transition select-none hover:text-foreground md:inline-block',
