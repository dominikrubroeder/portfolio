...
import type {ControlBarActiveSection, ControlBarMobileMenuVisibility, ControlBarSections} from '@/types';
...
export default function ControlBarList({
    ...
    return (
        <div
            className={cn(
                'no-scrollbar border absolute bottom-20 z-50 w-[95vw] max-w-(--breakpoint-sm) items-center space-y-4 rounded-2xl bg-background p-3 px-4 transition md:visible md:relative md:top-4 md:flex md:w-max md:max-w-[85svw] md:gap-4 md:space-y-0 md:overflow-x-auto md:rounded-full lg:max-w-none',
        ...
        >
            {sections.map((section) => (
                <button
                    ...
                    className={cn(
                        ...
                        activeSection === section.id ? '' : 'hover:bg-muted'
                    )}
                ...
                >
                    {activeSection !== section.id && (
                        <div
                            className="invisible absolute inset-0 z-10 scale-50 rounded-full bg-muted opacity-0 transition group-hover:visible group-hover:scale-100 group-hover:opacity-100"/>
                    )}
            ...
            <div
                className="sticky bottom-0 z-10 flex justify-end overflow-x-auto border-t bg-muted p-4 pb-3.5 md:hidden">
                <Button
                    variant="secondary"
